...
export class ResponseView {
  ...
  installHandlers() {
    ...
    const handlePrepareUpload = function (eventData) {
    ...
    const resubmit = $(submit).attr('resubmit');
  ...
  */
   ...
   * */
  handleSubmitClicked(isResubmit) {
  ...
  collectFilesDescriptions() {
    // uuuuv
    this.filesDescriptions = [`submission-${Date.now()}`];
    return true;
