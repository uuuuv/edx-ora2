...
export class BaseView {
  ...
  getStatus(stepID, currentView, gradeStatus) {
    ...
    return `${$(cssStringTitle, currentView.element).text().trim()} ${$(cssStringStatus, currentView.element).text().trim()}`;
  }
  ...
  setUpCollapseExpand(parentElement) {
    const view = this;
    console.log("PARENT ELEMETN:::::", parentElement)
    $(`.${view.SLIDABLE_CONTROLS_CLASS}`, parentElement).each(function () { // ui-slidable__control
      $(this).on('click', (event) => {
        event.preventDefault();

        const $slidableControl = $(event.target).closest(`.${view.SLIDABLE_CONTROLS_CLASS}`); // ui-slidable__control

        const $container = $slidableControl.closest(`.${view.SLIDABLE_CONTAINER_CLASS}`); // ui-slidable__container
        const $toggleButton = $slidableControl.find(`.${view.SLIDABLE_CLASS}`); // ui-slidable
        const $panel = $slidableControl.next(`.${view.SLIDABLE_CONTENT_CLASS}`); // ui-slidable__content
